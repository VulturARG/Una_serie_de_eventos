...
//		{
			...
			if(
				(
					(_showAIs && {!(_unit call _isPlayer)} && {0=={ {player==_unit} count crew player>0} count allUnitsUav}) ||
			...
			if(_show) then {
				...
				if(_vehicle == vehicle _player) then {
					_marker setMarkerSizeLocal [0.8,1]; // Para mil_triangle
					//_marker setMarkerSizeLocal [0.8,0.8];
				} else {
					_marker setMarkerSizeLocal [0.5,0.7]; // Para mil_triangle
					//_marker setMarkerSizeLocal [0.5,0.5];
				};
				_markerText setMarkerTextLocal format["%1",mapGridPosition  player];
	 			...
	 			/*if(_vehicle != _unit && !(_vehicle isKindOf "ParachuteBase")) then {
					...
					if((aero_player_markers_pos distance getPosATL _vehicle) < 50) then {
						...
						_num = 0;
						{
							if(alive player && player call _isPlayer && player != driver _vehicle) then {
								_text = format["%1%2 %3", _text, if(_num>0)then{","}else{""}, name player];
								_num = _num + 1;
							};
						} forEach crew _vehicle;
					} else {
						_num = {alive player && player call _isPlayer && player != driver _vehicle} count crew _vehicle;
						if (_num>0) then {
							if (isNull driver _vehicle) then {
